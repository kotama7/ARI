...
4 つの直交する機構を組み合わせる。 いずれも純粋に ARI 側の
...
\begin{enumerate}
  ...
  \item \emph{Step 4 再現パッケージ生成器} を
    \code{ari-skill-replicate/src/reproduce\_plan.py} の
    \texttt{generate\_reproduce\_plan} MCP ツールとして提供。 LLM
    が論文 (および存在すれば AD/AE Appendix concat 版) を読み、
    対象ディレクトリに 4 つの成果物を書き出す:
    \texttt{reproduce\_plan.md} (実験ごとの再現性カテゴリ付き
    段階的再構成手順)、 \texttt{verification\_code.py} (論文の
    数値主張に対する許容誤差付き整合性チェック雛形)、
    \texttt{install\_commands.txt} (paper / AD / AE から抽出した
    shell コマンド)、 \texttt{reproduce.log} (論文自身の報告値で
    合成した模擬実行ログ)。 このディレクトリを
    \texttt{judge\_submission} の \texttt{submission\_dir} として
    渡すことで、 \texttt{SimpleJudge} の \texttt{Result Analysis}
    分岐に評価対象が供給される。 同梱プロンプトは venue 非依存
    (regression test で強制)、 venue 固有の再現性基準は rubric
    生成と同じ YAML テンプレートに閉じる。
  ...
  \item \emph{段階的入力条件}。 dogfood スクリプト
    ...
    / \texttt{paper+AD+AE+metadata} 各条件の score を測定でき、
    各成果物層の寄与を分解できる。
\end{enumerate}

4 機構の合計効果は加算超過 (super-additive) であり、 単独最大は
プロンプトパッチ (機構 2)、 次が AD/AE concat (機構 4)、
