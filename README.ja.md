...
ビジュアルな実験管理のための 9 ページ構成 React/TypeScript SPA。起動方法：
...
| **New Experiment** | 4 ステップウィザード: チャット/記述/アップロードでゴール設定 → スコープ（深さ・ノード数・ワーカー）→ リソース（LLM・HPC）→ 起動 |
...
| **Results** | 論文（PDF/TeX）の表示/ダウンロード、レビューレポート、再現性結果、生成された図 |
| **Ideas** | VirSci 生成の仮説、新規性/実現可能性スコア、ギャップ分析 |
| **Workflow** | BFTS 後パイプラインステージの編集（依存関係、有効/無効、入出力） |
| **Settings** | LLM プロバイダ/モデル、API キー（.env ベース）、SLURM パーティション自動検出、SSH リモートテスト、Ollama ホスト |
...
| `/api/settings` | GET/POST | LLM・SLURM・API キー設定の読み書き |
...
| `review_report.json` | 自動ピアレビュースコアとフィードバック |
...
合計 14 スキル。9 つは `workflow.yaml` でデフォルト登録され、残り 5 つは設定に追加することで有効化できます。
...
| `ari-skill-web` | DuckDuckGo, arXiv, Semantic Scholar, 反復引用収集 | △ | ✓ |
| `ari-skill-memory` | 祖先スコープのノードメモリ（JSONL） | ✗ | ✓ |
| `ari-skill-transform` | BFTS ツリー → 科学向けデータ形式 | ✓ | ✓ |
| `ari-skill-plot` | Matplotlib/seaborn 図生成 | ✓ | ✓ |
| `ari-skill-paper` | LaTeX 執筆 + BibTeX + レビュー | ✓ | ✓ |
| `ari-skill-paper-re` | ReAct 再現性検証 | ✓ | ✓ |
| `ari-skill-coding` | コード生成 + 実行 + bash | ✗ | — |
| `ari-skill-benchmark` | CSV/JSON 分析、プロット、統計検定 | ✗ | — |
| `ari-skill-review` | ピアレビュー解析、リバッタル生成 | ✓ | — |
| `ari-skill-vlm` | Vision-Language モデルによる図/表レビュー | ✓ | — |
| `ari-skill-orchestrator` | 外部エージェント向けに ARI を MCP サーバとして公開 | ✗ | — |
...
| P2 | 可能な限り決定論的 | MCP ツールはデフォルトで決定論的、LLM 使用ツールは明示的に注釈 |
| P3 | 多目的メトリクス | ハードコードされたスカラースコアなし |
| P4 | 依存性注入 | 実験の切り替え = `.md` の編集のみ |
| P5 | 再現性ファースト | 論文ではクラスタ名ではなくスペックでハードウェアを記述 |
