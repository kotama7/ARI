...
| **New Experiment** | 4 ステップウィザード: チャット/記述/アップロードでゴール設定 → スコープ（深さ・ノード数・ワーカー・再帰深度）→ リソース（LLM・HPC・コンテナ）→ 起動 |
...
| **Workflow** | React Flow ビジュアル DAG エディタ（ドラッグ・接続・有効/無効・スキル割り当て） |
| **Settings** | LLM プロバイダ/モデル、API キー、SLURM、コンテナランタイム、VLM レビューモデル、検索バックエンド、Ollama ホスト |
...
| `rebuttal.json` | レビューコメントへの自動リバッタル |
...
合計 15 スキル。14 つは `workflow.yaml` でデフォルト登録され、残り 1 つ（orchestrator）は設定に追加することで有効化できます。
...
| `ari-skill-figure-router` | 図種分類・生成ルーティング（SVG/matplotlib/LaTeX） | ✓ | ✓ |
