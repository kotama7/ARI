...
def profile_node(work_dir: str, *, seed: int = 0, shape=None,
    ...
    """Counters for the candidate's SCORED region on one gemm shape.
    ...
    try:
        ...
        def _run_one(exe, td, launcher, cap):
            ti, _tw, _C = _run_exe(exe, td, A, B, launcher=launcher, capture=cap)
            return ti

        return _profile_common(
            task="gemm", work_dir=work_dir, counters=counters, digest=digest,
