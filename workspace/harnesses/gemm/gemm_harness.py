...
def _compile_kernel(kind: str, work_dir: str, out_td: str,
    ...
    # gemm_main_profiled.c, which is this driver plus a counter gate; the
    ...
    main_c = main_src or _os.path.join(kdir, "gemm_main.c")
    _tag = tag or kind
    _PKG_KERNELS = {
    ...
    if kind == "candidate":
        import shutil as _sh
        src_c = _os.path.join(out_td, f"candidate_{_tag}_gemm.c")
...
def _profile_record(task: str, work_dir: str, points: list, digest: str,
    ...
    return {
        ...
        "threads": _os_pin.environ.get(
            "ARI_GEMM_THREADS", _os_pin.environ.get("OMP_NUM_THREADS", "")),
...
def profile_node(work_dir: str, *, seed: int = 0, reps: int = 3, cases=None,
                 shapes=None, line_bytes: int | None = None) -> dict:
    """Counters for the candidate's SCORED region, over chosen shapes and reps.

    ``cases``/``shapes`` — which scored shapes to profile; default ALL of them,
    so the profile covers what the score covers. ``reps`` — independent
    processes per shape, with the scored input-seed formula, so point r lines up
    with scored repetition r.
    """
    import os as _o
    import tempfile as _tf

    chosen = cases if cases is not None else shapes
    use = tuple(tuple(s) for s in chosen) if chosen else tuple(SHAPES)
    ...
    try:
        ...
        exe = _compile_kernel("candidate", work_dir, td_obj.name,
                              ...
                              main_src=_o.path.join(kernels_dir(),
                                                    "gemm_main_profiled.c"),
                              tag="candidate_profiled")

        # The candidate's process may need its selected compiler's runtime on
        # the loader path -- without it a vendor-built candidate dies on a
        # missing shared object. measure_node has always passed this; the
        # profile did not, so exactly the candidates whose toolchain the profile
        # is most interesting for could not be profiled at all.
        _cand_libs = _runtime_libs_for(
            (_select_candidate_cc(work_dir) or {}).get("resolved_path"))

        def _run_one(shape, input_seed, launcher, cap):
            A, B = gen_problem(shape, seed=input_seed)
            ti, _tw, C = _run_exe(exe, td_obj.name, A, B, launcher=launcher,
                                  capture=cap, ld_library_path=_cand_libs)
            ok, rel = is_correct(C, reference_gemm(A, B), A, B)
            return ti, bool(ok), float(rel)

        points = []
        for shape in use:
            name = "x".join(str(v) for v in shape)
            for r in range(int(reps)):
                points.append(_profile_point(_run_one, shape,
                                             seed * 100003 + r, launcher, name))
        return _profile_record("gemm", work_dir, points, digest,
                               {"shapes": [list(s) for s in use], "reps": int(reps),
                                "seed": int(seed)})
    finally:
        td_obj.cleanup()
