...
# translation unit ONLY. The reference is defined as "competent AND vectorised
# for the target ISA"; on the scoring target -march=native emits no SVE, so
# without this flag the frozen reference would silently be a weaker
# implementation than the one the score scale was calibrated against.
#
# The default is chosen by ISA rather than hardcoded, because the login node is
# x86_64 while the compute nodes are aarch64 and the harness must stay
# importable and testable on both. Whichever set is used is RECORDED in the
# result, so a run built with the weaker reference is visible in the data
# instead of silent. Overridable only to port the harness, never to retune it.
...
_REFERENCE_CFLAGS: tuple[str, ...] = tuple(
    _os_pin.environ.get("ARI_SPMM_REF_CFLAGS", _default_reference_cflags()).split()
)
...
def _matrix_cache_dir():
    """Where generated matrices are cached, or None to disable caching.

    ``ARI_HARNESS_CACHE`` should point at storage shared by every Slurm array
    element, because the win comes from reuse ACROSS arms and nodes, not within
    one node.
    """
    import os as _os
...
def _gen_matrix_cached(family: str, n: int, *, density: float = 0.02,
                       seed: int = 0):
    """``gen_matrix`` with a content-verified disk cache.

    gen_matrix sits OUTSIDE the repetition loop and its arguments do not depend
    on the arm, the node or the repetition, so the v1 campaign regenerated the
    same six matrices for every one of its 2,700 nodes — a fixed per-node cost
    the cost audit put at 58.5 s, roughly 44 h of the campaign. The matrix is the
    scored problem, so the cache is keyed on every argument that determines it
    AND re-checks those arguments after loading; anything unexpected falls
    through to regeneration rather than scoring a different problem.
    """
    ...
    def _content_digest(M):
        h = _hl.sha256()
        for a in (M.indptr, M.indices, M.data):
            h.update(_np.ascontiguousarray(a).tobytes())
        return h.hexdigest()

    cdir = _matrix_cache_dir()
    if cdir is None:
        return gen_matrix(family, n, density=density, seed=seed)
    key = f"spmm_{family}_n{n}_d{density!r}_s{seed}.npz"
    path = _os.path.join(cdir, key)
    if _os.path.isfile(path):
        try:
            with _np.load(path, allow_pickle=False) as z:
                if (str(z["family"]) == family and int(z["n"]) == n
                        and float(z["density"]) == float(density)
                        and int(z["seed"]) == int(seed)):
                    A = sp.csr_matrix(
                        (z["data"], z["indices"], z["indptr"]),
                        shape=(int(z["rows"]), int(z["cols"])))
                    # v2: verify the CONTENT, not just the key. scipy's RNG
                    # consumption is an implementation detail, so a cache
                    # written by a different scipy would otherwise score a
                    # different problem under the same reported family/seed.
                    if (A.shape == (n, n) and A.nnz == int(z["nnz"])
                            and _content_digest(A) == str(z["sha256"])):
                        return A
        except Exception:
            pass  # unreadable or stale: regenerate below
    A = gen_matrix(family, n, density=density, seed=seed)
    try:
        # np.savez APPENDS .npz when the name lacks it, so the temp name must
        # already end in .npz or the rename below looks for a file that was
        # never written - which silently disabled this cache entirely.
        tmp = f"{path}.{_os.getpid()}.tmp.npz"
        _np.savez(tmp, data=A.data, indices=A.indices, indptr=A.indptr,
                  rows=A.shape[0], cols=A.shape[1], nnz=A.nnz,
                  family=family, n=n, density=density, seed=seed,
                  sha256=_content_digest(A))
        _os.replace(tmp, path)   # atomic: concurrent array elements cannot tear
    except Exception as exc:
        # Never fail the measurement over a cache problem, but do not hide it
        # either: a silently-never-hitting cache is worse than none, because
        # it pays the write cost on every node and saves nothing.
        if not _gen_matrix_cached.__dict__.get("_warned"):
            _gen_matrix_cached.__dict__["_warned"] = True
            print(f"[spmm harness] matrix cache write failed ({exc!r}); "
                  f"continuing without it", file=_sys.stderr)
    return A


_SELFTEST_POINTER = ".selftest_problems"


def _materialise_selftest_problems(n: int = 20000, k: int = 64,
                                   density: float = 0.02, seed: int = 0):
    """Write the six SCORED matrices where the C selftest can read them.

    The selftest could never match the evaluator by generating its own matrices:
    four of the six families come out of scipy's RNG and cannot be reproduced in
    C at all, which is why v1 ran only 2 of 6 and its transfer slope to the
    scored value was 0.365. Writing the real problems once, in the same binary
    layout ``spmm_main.c`` reads, makes the local number measure the same thing
    the score does.

    They go in the shared cache (ARI_HARNESS_CACHE), not the node work_dir: six
    families are on the order of a gigabyte and every node would otherwise carry
    its own copy. Returns the directory, or None when no cache is configured, in
    which case the selftest falls back to generating and says so.
    """
    import os as _os
    import numpy as _np
    ...
    if cdir is None:
        return None
    outdir = _os.path.join(cdir, f"selftest_problems_n{n}_k{k}_s{seed}")
    try:
        _os.makedirs(outdir, exist_ok=True)
    except OSError:
        return None
    rng = _np.random.default_rng(seed + 7919)     # fixed X: the selftest is a
    X = rng.standard_normal((n, k)).astype(_np.float64)   # predictor, not the score
    for fam in FAMILIES:
        path = _os.path.join(outdir, f"{fam}.bin")
        if _os.path.isfile(path):
            continue
        A = _gen_matrix_cached(fam, n, density=density, seed=seed)
        tmp = f"{path}.{_os.getpid()}.tmp"
        try:
            with open(tmp, "wb") as fh:
                _np.array([A.shape[0], A.shape[1], k, A.nnz],
                          dtype=_np.int32).tofile(fh)
                A.indptr.astype(_np.int32).tofile(fh)
                A.indices.astype(_np.int32).tofile(fh)
                A.data.astype(_np.float64).tofile(fh)
                X.tofile(fh)
...
def is_correct(Y_cand: np.ndarray, Y_ref: np.ndarray, A: sp.csr_matrix,
               X: np.ndarray, *, C: float = 8.0, u: float = _FP64_U,
               bound_key: str | None = None) -> tuple[bool, float]:
    ...
    # |A| @ |X| is a SECOND sparse product the same size as the reference one,
    # and it runs on every repetition. Measured on the compute node at the
    # scored size, the two Python products together are about 80% of the 4.06 s
    # a repetition costs, against 0.013 s for the kernel actually being scored.
    # It is a deterministic function of the problem, so it is cached under the
    # same key — and, like the reference itself, VERIFIED on every hit: an
    # inflated bound would silently accept wrong output.
    absX = np.abs(np.asarray(X, dtype=np.float64))
    prod = _cached_abs_product(absA, absX, bound_key)
    bound = C * g[:, None] * prod
    resid = np.abs(Yc - Yr)
...
def seed_work_dir(work_dir: str) -> list[str]:
    ...
    # v2: tell the selftest where the SCORED matrices are, so its number
    # predicts the score instead of measuring different matrices.
    try:
        _probdir = _materialise_selftest_problems()
    except Exception:
        _probdir = None
    if _probdir:
        with open(_os.path.join(work_dir, _SELFTEST_POINTER), "w",
                  encoding="utf-8") as _fh:
            _fh.write(_probdir + "\n")
        written.append(_SELFTEST_POINTER)
...
def _compile_kernel(kind: str, work_dir: str, out_td: str,
    ...
    # spmm_main_profiled.c, which is this driver plus a counter gate; the
    ...
    main_c = main_src or _os.path.join(kdir, "spmm_main.c")
    _tag = tag or kind
    _PKG_KERNELS = {
        "reference": "reference_spmm.c",
        "baseline": "baseline_spmm.c",
    }
    ...
    if kind == "candidate":
        import shutil as _sh
        src_c = _os.path.join(out_td, f"candidate_{_tag}_spmm.c")
        _sh.copy2(kern_c, src_c)

    _cc_default = _os.environ.get("ARI_SPMM_CC", "cc")
    ...
    # _isa_flags_for. ARI_SPMM_CFLAGS still overrides everything.
    _env_cflags = _os.environ.get("ARI_SPMM_CFLAGS")
    ...
    if extra:
        exc = CandidateInvalidError if kind == "candidate" else HarnessInfrastructureError
        raise exc(
            f"{kind} kernel exports symbols other than 'spmm': {extra[:8]}")
...
def _warn_once(msg: str) -> None:
    ...
    A cache miss or a failed check must be VISIBLE — silently falling back to
    recomputation hides a broken cache that is quietly costing what it was
    added to save — but repeating it on every repetition would bury the run log.
    """
    import sys as _sys
    ...
    if msg in seen:
        return
    seen.add(msg)
    print(f"[spmm_harness] {msg}", file=_sys.stderr, flush=True)



def _cached_abs_product(absA, absX, key: str | None):
    """``absA @ absX`` with a disk cache, verified on every hit.

    Same contract as _cached_reference_spmm: this feeds the correctness bound,
    so a wrong entry would loosen the oracle rather than fail, and the row check
    is what stops that. Kept separate from the reference cache because the two
    are different arrays and a single file would have to be invalidated as a
    unit.
    """
    import os as _os
    ...
    if not cdir:
        return absA @ absX
    path = _os.path.join(cdir, f"spmm_absprod_{key}.npy")
    n = int(absA.shape[0])
    k = int(absX.shape[1])
    if _os.path.isfile(path):
        try:
            P = np.load(path)
            if P.shape == (n, k) and P.dtype == np.float64:
                i = (hash(key) % n) if n else 0
                row = np.asarray(absA[i].dot(absX)).ravel()
                if np.allclose(row, P[i], rtol=1e-12, atol=1e-12):
                    return P
                _warn_once(f"bound cache row check FAILED for {key}; recomputing")
            else:
                _warn_once(f"bound cache shape/dtype mismatch for {key}; recomputing")
        except Exception as exc:
            _warn_once(f"bound cache unreadable for {key} ({exc}); recomputing")
    P = absA @ absX
    try:
        ...
        _warn_once(f"bound cache write failed ({exc}); continuing uncached")
    return P


def _cached_reference_spmm(A, X, key: str | None):
    """``reference_spmm(A, X)`` with a disk cache, VERIFIED on every hit.

    This is the correctness oracle, so a stale or corrupt cache entry would not
    fail — it would silently accept wrong candidate output, which is the worst
    failure this harness can have. Measured cost is 0.547 s per repetition at
    the scored size (8M nonzeros, k=64), which is 49 s of the 365 s a reps=15
    scoring takes, so it is worth caching but only with a check.

    The check recomputes ONE row of the product and compares it against the
    cached row. That is 1/20000 of the work and catches truncation, a shape or
    dtype mismatch, and a key collision. Any mismatch discards the entry and
    recomputes from scratch rather than trusting it.
    """
    import os as _os

    if not key:
        return reference_spmm(A, X)
    cdir = _matrix_cache_dir()
    if not cdir:
        return reference_spmm(A, X)
    path = _os.path.join(cdir, f"spmm_yref_{key}.npy")
    n = int(A.shape[0])
    k = int(np.asarray(X).shape[1])
    if _os.path.isfile(path):
        try:
            Y = np.load(path)
            if Y.shape == (n, k) and Y.dtype == np.float64:
                i = (hash(key) % n) if n else 0
                row = np.asarray(A[i].dot(np.asarray(X, dtype=np.float64))).ravel()
                if np.allclose(row, Y[i], rtol=1e-12, atol=1e-12):
                    return Y
                _warn_once(f"reference cache row check FAILED for {key}; recomputing")
            else:
                _warn_once(f"reference cache shape/dtype mismatch for {key}; recomputing")
        except Exception as exc:
            _warn_once(f"reference cache unreadable for {key} ({exc}); recomputing")
    ...
    try:
        ...
        _warn_once(f"reference cache write failed ({exc}); continuing uncached")
    return Y
...
def _run_exe(exe: str, work_td: str, A, X, problem_key: str | None = None,
    ...
    only caps the candidate's parallelism. The study uses 48 threads on the scored partition;
    ...
    # The problem file is ~106 MB at the scored size (32 MB of indices, 64 MB of
    # values, 10 MB of X) and was written afresh for EVERY process launch. Within
    # one repetition the candidate and the reference are handed the SAME (A, X),
    # so it was written twice identically; and because the seeds are a
    # deterministic function of the repetition index, every node in the campaign
    # regenerated the same files again. At 36 launches per scoring that is 3.8 GB
    # of writes per node, which measured as 84 s of the 84.2 s spent scoring at
    # reps=3 - the kernel itself is under 100 ms.
    #
    # The file is content-determined by (family, n, k, seed, input_seed), so it is
    # cached under ARI_HARNESS_CACHE and HARD LINKED into the run directory. The
    # anti-gaming properties are unchanged: the path the scored process sees and
    # the bytes behind it are exactly what they were before, and the problem is
    # still fresh per repetition because input_seed is part of the key.
    expect_bytes = (16 + Acsr.indptr.nbytes + Acsr.indices.nbytes
                    + Acsr.data.nbytes + Xc.nbytes)
    cached = None
    ...
    if problem_key and _os.environ.get("ARI_CACHE_PROBLEM_FILES", "0") == "1":
        cdir = _matrix_cache_dir()
        if cdir:
            cached = _os.path.join(cdir, f"spmm_prob_{problem_key}.bin")
            try:
                ...
                cached = None          # fall through to a direct write
    try:
    ...
    if cached:
        try:
            _os.link(cached, prob)     # same bytes, no copy, no write
            linked = True
        except OSError:
            try:
                import shutil as _sh
...
def _measure_node_once(
    ...
    *,
    ...
    _become_subreaper()
    # Set per repetition; read by the run() closure so its arity (and therefore
    # the injected-runner test seam) does not change.
    ...
    try:
        for fam in families:
            ...
            for r in range(n_reps):
                ...
                _problem_key[0] = f"{fam}_n{n}_k{k}_s{seed}_r{input_seed}"
                X = np.random.default_rng(input_seed).standard_normal((A.shape[1], k))
                Y_ref = _cached_reference_spmm(A, X, _problem_key[0])
                ...
                for kind in order:
                    try:
                        ...
                        if kind == "reference":
                            raise HarnessInfrastructureError(
                                f"reference run failed on {fam}, rep {r}: {exc}") from exc
                        valid = False
                        evaluation_status = "candidate_invalid"
                ...
                if "reference_matched" in exes:
                    try:
                        observed["reference_matched"] = _unpack_run(
                            run("reference_matched", work_dir, A, X))
                    except Exception as exc:
                        rec["measurements"]["reference_matched_error"] = str(exc)

                tb, tb_wall, Y_ref_run = observed["reference"]
                ...
                if r == 0:
                    ref_ok, ref_mr = is_correct(Y_ref_run, Y_ref, A, X,
                                                bound_key=_problem_key[0])
                    if not ref_ok:
                        raise HarnessInfrastructureError(
                            f"the FROZEN REFERENCE is incorrect on {fam} at the "
            ...
            if n_implausible >= 2:
                ...
                evaluation_status = "candidate_invalid"
                reason = (
                    f"implausible timing on {fam}: {n_implausible}/{n_reps} reps "
    ...
    return {
        ...
        # reference was built without the target's vector flags is visible in
        # the data rather than silently inflating every score.
...
def measure_node(work_dir: str, **kwargs) -> dict:
    ...
    src = _os.path.join(snap, "candidate_spmm.c")
    ...
    submitted = _os.path.join(work_dir, "candidate_spmm.c")
...
def measurement_placement() -> dict:
    ...
    threads = _os_pin.environ.get("ARI_SPMM_THREADS",
