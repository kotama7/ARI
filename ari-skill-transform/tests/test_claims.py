...
def _typed(good: list[dict]) -> dict[str, dict]:
    return {
        node["id"]: {
            "measurements": {
                key: value
...
def test_formula_semantics():
    ...
    assert (
        recompute("relative_reduction_percent", {"baseline": 10.0, "proposed": 8.0})
        == 20.0
    )
    assert (
        recompute("relative_increase_percent", {"baseline": 8.0, "proposed": 10.0})
        == 25.0
    )
...
def test_recompute_guards():
    ...
    assert (
        recompute("relative_reduction_percent", {"baseline": 0.0, "proposed": 1.0})
        is None
    )
...
def test_resolve_metric_rejects_legacy_node_metrics():
    node = {"id": "n1", "metrics": {"GFlops": 99.0}}
    val, path = _resolve_metric(node, {}, "GFlops")
    assert val is None and path == ""


def test_resolve_metric_rejects_untyped_scores():
    node = {"id": "n1", "metrics": {}}
...
def test_higher_is_better_absolute_and_comparison():
    ...
    out = build_science_claims(
        good, _typed(good), primary_metric="GFlops", higher_is_better=True
    )
    claims = out["claims"]
    ...
    assert na["operands"]["value"]["metric_path"] == "measurements.GFlops"
...
def test_lower_is_better_uses_reduction():
    ...
    out = build_science_claims(
        good, _typed(good), primary_metric="runtime", higher_is_better=False
    )
    claims = out["claims"]
...
def test_flattened_numeric_assertions_have_claim_id():
    ...
    out = build_science_claims(
        good, _typed(good), primary_metric="x", higher_is_better=True
    )
...
def test_no_resolvable_metric_returns_empty():
    ...
    out = build_science_claims(
        good, {}, primary_metric="missing", higher_is_better=True
    )
...
def test_results_json_operands_are_deterministic():
    ...
    assert a == b
    assert (
        a["claims"][0]["numeric_assertions"][0]["operands"]["value"]["metric_path"]
        == "measurements.GFlops"
    )
...
def test_prose_primary_metric_falls_back_to_autodetect():
    ...
    prose = (
        "Effective GFLOP/s averaged across RHS widths K in {8,16,32,64}, "
        "geometric mean speedup over a tuned baseline CSR-SpMM, with roofline ratio"
    )
    out = build_science_claims(
        good, _typed(good), primary_metric=prose, higher_is_better=True
    )
    assert len(out["claims"]) >= 1
...
def test_operands_tagged_with_environment():
    ...
    env = {
        "a": {"executor": "local", "cpu_model": "Intel", "arch": "x86_64"},
        "b": {"executor": "slurm", "cpu_model": "cpuX", "arch": "aarch64"},
    }
    out = build_science_claims(
        good, _typed(good), "x", True, node_env=env, comparison_scope="any"
    )
...
def test_same_environment_scope_keeps_same_env_comparison():
    ...
    env = {
        "a": {"executor": "slurm", "cpu_model": "cpuX", "arch": "aarch64"},
        "b": {"executor": "slurm", "cpu_model": "cpuX", "arch": "aarch64"},
...
def test_ratio_percent_formula():
    assert (
        round(recompute("ratio_percent", {"baseline": 114.0, "proposed": 81.55}), 1)
        == 71.5
    )
    assert recompute("ratio_percent", {"baseline": 0.0, "proposed": 1.0}) is None


def test_transform_uses_the_canonical_formula_registry_object():
    from ari.public.science_data import FORMULAS as canonical_formulas
    from ari.public.science_data import recompute as canonical_recompute

    assert FORMULAS is canonical_formulas
    assert recompute is canonical_recompute
