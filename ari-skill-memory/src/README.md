...
- `ari_skill_memory/` — importable library package: the memory backend abstraction.
  ...
  - `erasure.py` — reader for ARI's published `rqgm_erasure_state.json` rollup: labels entries from logically erased nodes (annotate, never hide) and hard-excludes them from the grounded-claims lists. No `ari` import, no LLM call, absence == nothing stale.
