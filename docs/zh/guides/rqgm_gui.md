...
`GET /api/checkpoint/{run_id}/kca`（用它自己的客户端取用），同样是 GET。其余
八个标签页都经由类型化的 `/api/v1` 客户端 hook 从那些 `rqgm/*` 读模型取数。
为页面供数的读取里，仅剩共享节点选择器的选项列表在这些之外 —— 它仅仅为了枚举
节点 id 而从运行树读模型（`GET /api/v1/runs/{run_id}/tree`）填充 —— 同样是
GET。你在这个工作区里点击任何东西，都不会注册组件、采纳策略、开启纪元或改写
得分。治理的变更来自运行
...
Governance 是一条 **v2 专属路由**，由外壳的 `gui_v2` 能力标志把关。当
`ARI_GUI_V2=0` 或 `false` 时，侧边栏会去掉 Governance 条目，而 `#/governance`
会像一个未知哈希那样解析到 Home —— 悄无声息，没有任何提示。这条回退位于本
工作区之上的外壳路由器里，因此它无法显示下文那个能力状态页面：与治理相关的
任何东西都不会挂载，也就没有谁能出面解释。实际上你只有在有人刻意打开这个
kill-switch 时才会遇到它 —— 该标志默认开启，而且 `GET /api/capabilities` 取用
失败时会被当作 `gui_v2: true`，而不是把你丢进回退外壳（见
[仪表盘指南 → 能力标志](dashboard.md#能力标志)）。
