...
1. **启动 Letta 服务。** 从
   `docs/hpc_setup.md#6-letta-memory-backend-deployment` 中选择
...
`docs/release_policy.md`）安排了以下移除项：
...
- `docs/refactor_audit.md` —— 迁移债务的当前状态。
- `CHANGELOG.md` —— 各版本发布说明。
- `ari memory migrate --help` —— v0.5 → v0.6 迁移工具的 CLI 选项。
- `docs/howto/troubleshooting.md` —— 迁移失败时的处理方法。
