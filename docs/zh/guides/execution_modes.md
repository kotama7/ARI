...
1. **运行开始时** —— 这是进入 `ari_rqgm` 的唯一入口。生效模式只解析一次，
   并在第一个节点运行前被持久化。
2. **纪元边界**（仅限 `ari_rqgm` 运行内部）—— 运行可以在纪元边界事务中
   **降级** `ari_rqgm → simple_bfts`（成本 / 紧急回退）。该降级会被审计
   并由 ConstitutionalKernel 校验。
...
运行中途翻转模式 —— 运行的模式除纪元边界（且只能降级）外不可变。
...
2. **仅限新建运行。** 两个叶子都是 `mutability: new_run_only`。该选择作用于
   ...
   运行的模式除纪元边界（且只能降级）之外保持不变。没有任何 GUI 路径会写入
   该文件。
