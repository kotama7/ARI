...
| Security regression suites | `python -m pytest ari-core/tests -q -k "gui_bind_cors or gui_path_proxy_hardening or gui_confirmation_challenges or gui_csp_headers or gui_remote_auth or gui_secret_readiness or gui_workflow_write_guard"` | exit 0 — these are the MN-1/2/4/5/6/7/8 refusals |
...
- **Dependency vulnerability scan.**  Both lockfiles exist:
  `ari-core/ari/viz/frontend/package-lock.json` on the JavaScript side, and the
  repo-root `requirements.lock` — a `uv pip compile requirements.txt` output
  that `scripts/setup/install_deps.sh` installs frozen — on the Python side.
  The Python lock covers the root `requirements.txt` only: the per-package
  `pyproject.toml` files still declare floating lower bounds (`>=`), and
  `.github/dependabot.yml` records that a merged pip bump leaves
  `requirements.lock` to be regenerated by hand, so it can drift.  Neither side
  is scanned for known vulnerabilities, by any workflow in `.github/workflows/`
  or by any row of the gate table above — Dependabot raises bump PRs, which is
  not a scan.  A pinned dependency is a *known* dependency, not a safe one.
...
|---|---|
| Route success/error, schema mismatch, adapter mismatch | browser devtools network panel; typed `/api/v1` responses carry a `request_id`, but `viz_access.jsonl` lines hold only `ts` / `method` / `path` / `status` / `duration_ms` / `client`, so correlating one back to a server line is by path and timestamp, not by that id |
