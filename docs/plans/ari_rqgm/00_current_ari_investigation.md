...
|---|---|---|
| Epoch boundary | Head of the outer `while` in `_run_loop` — the only natural round boundary; sees `all_nodes` / `frontier` / `pending` / `cfg` / `checkpoint_dir` on the main thread. Expansion and execution interleave, so "epoch = N outer iterations vs N completed nodes vs a lineage event" is an open decision. | 01, 02, 05 |
...
| Q-21 | Epoch boundary granularity (N outer iterations / N nodes / lineage event); does the epoch layer subsume, veto, or coexist with the lineage-decision hook — shared rate limit and audit log, or separate `rqgm_*` ones? | Task 05 |
