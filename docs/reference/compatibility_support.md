...
| PaperBench runtime adaptations | Only adaptations listed in `paperbench_patches.json` at the exact source pin are applied and conformance-tested. | Paper-re maintainers | Re-evaluate on every PaperBench pin update; remove an adaptation when its `obsolete` probe passes and the target-version suite is green. |
