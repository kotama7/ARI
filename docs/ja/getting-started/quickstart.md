...
CLI（コマンドライン）での使い方については、[CLI リファレンス](../reference/cli_reference.md)を参照してください。

> **インストール前にプレビュー**
>
> - 🎬 **ダッシュボードのデモ動画** — [`movie/ja/ari_dashboard_demo.mp4`](../../assets/movie/ja/ari_dashboard_demo.mp4) で Web UI の動作を一通り確認できます。
> - 📄 **サンプル成果物（論文）** — [`sample_paper.pdf`](../../assets/sample_paper.pdf) は ARI が実際に生成した論文です。図表・引用・再現性検証レポートまで含まれています。
...
- **ARI の拡張:** 新しいスキルの追加については [拡張ガイド](../guides/extension_guide.md) を参照
