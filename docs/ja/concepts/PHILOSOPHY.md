...
## メモリ (v0.6.0): P2 は 1 つの skill について緩和、P5 はスコープ限定

ARI の当初の設計は **P2 — 可能な限り決定的に** と **P5 — 再現性優先** を
掲げていました。v0.6.0 は決定的な JSONL メモリストアを
[Letta](https://docs.letta.com) に置き換え、埋め込みベースの検索と本格的な
エージェントメモリ管理を使えるようにしました。システム内で P2 が緩和される
のはここだけであり、その影響は限定され、文書化されています。

- **維持されるもの。** 数値としての実験結果は、同じシードであれば再現します。
  保存されたメモリの*テキスト*はバイト安定です: 祖先エントリは書き込み時点で
  Copy-on-Write により保護され、Letta の self-edit パスは既定で無効です
  (`ARI_MEMORY_LETTA_DISABLE_SELF_EDIT=true`)。
- **変わりうるもの。** 検索は埋め込みの浮動小数点演算とベクトルインデックスの
  状態に依存するため、BFTS の*軌跡*（どのノードをどの順で探索するか）は再実行
  のたびに分岐しうります。その事実は各検索自身が携行します:
  `MemoryRetrievalV1.provenance` が `backend`（`"letta"`）、`backend_version` /
  `server_version`、埋め込みの `model` / `model_version`、`ranking` 規則、
  そして `deterministic: false` を記録します。さらにメモリの読み書きはすべて
  `{checkpoint}/memory_access.jsonl` に追記されるため、軌跡の分岐はそれを
  生んだ検索まで遡って追跡できます。
- **なぜこのトレードオフか。** 決定的なキーワードスコアラは動作していましたが、
  実験をまたいだ推論にはスケールしませんでした。Letta は構造化されたコアメモリ、
  ベクトル検索、そして統一されたエージェント/コレクションモデルをもたらし、
  各 skill が検索を再実装せずに同じメモリ面を使えるようにします。

運用面では、Letta はローカル（Docker / Singularity / pip）または Letta Cloud で
動かす依存コンポーネントです。セットアップは `docs/reference/configuration.md`
と `ari memory` CLI を参照してください。

## 関連

[アーキテクチャ](architecture.md) · [BFTS アルゴリズム](bfts.md)
