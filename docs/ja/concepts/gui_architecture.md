...
- `Sidebar.tsx` は `navItems()` からナビ表を構築し、`navReplaces` エントリを
  ターゲットのスロットへ実体化して `navOrder` でソートします。
...
レガシー経路がこの規則の動機となる対比です。ランの行からレガシー Results ページ
へ至る経路は今も*暗黙の*受け渡し（`sessionStorage` のキー + 裸の `#/results`
ハッシュ）を通り、レガシーシェルはさらにプロセス全体の*アクティブチェックポイント*
を持ちます。どちらも v2 ワークスペースの妥当な入力ではありません: v2 ルートは
run を明示するからこそ、2 つのタブで 2 つのランを開いても互いを踏まないのです。
