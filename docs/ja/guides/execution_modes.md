...
1. **ラン開始時** — `ari_rqgm` への唯一の入口。実効モードは一度だけ解決され、
   最初のノードが走る前に永続化されます。
2. **エポック境界**（`ari_rqgm` ラン内のみ）— ランはエポック境界トランザクション
   の中で `ari_rqgm → simple_bfts` へ**ダウングレード**できます
   （コスト/緊急フォールバック）。ダウングレードは監査され、
   ConstitutionalKernel によって検証されます。
...
2. **新規ラン専用。** どちらの葉も `mutability: new_run_only` です。選択は
   ...
   モードはエポック境界（ダウングレードのみ）を除いて不変のままです。そのファイル
   を書く GUI パスは存在しません。
...
- ランのモードは開始後、どの面からも変更できません — resume は永続化された
  モードを取り、ラン中の唯一の遷移はエポック境界でのダウングレードです。
