...
カテゴリで絞り込むので、205 フィールドすべてが発見可能なままです。リゾルバの警告は
...
フィルタボックス横のカウンタは 205 より大きくなることがあります。解決済み
...
で編集してください（カスタムエディタは未対応）。」）。利用できないコントロールは
常に理由を述べ、黙って隠されることはありません。
...
ページはレジストリのエントリを読み、最初に一致したところで止まります:

1. `sensitivity` が `secret_reference` → 書き込み専用シークレットコントロール;
2. 空でない `enum` → セレクト;
3. それ以外は `value_type` の文字列を `|` で分割し `None` を落とす。残った
   メンバーがちょうど **1 つ**なら: `bool` → スイッチ、`int`/`float` →
   数値入力、`str` → テキスト入力;
4. それ以外はすべて複合 — 分割後にメンバーが 2 つ以上残る場合、または
   `list[…]` / `dict[…]` / ネストしたモデルの注釈。

つまり `sensitivity` が `enum` に優先し、`enum` が型に優先します。enum 値を持つ
シークレットであってもシークレットコントロールになり、整数値の enum であっても
数値入力ではなくセレクトになります。

**既知のギャップ: `field path/pattern → custom editor` のレジストリは存在しません。**
このワークスペースの GUI 計画はそれを求めていました — ドメイン固有のフィールドが、
ページを書き換えるのではなく自ら宣言することでより豊かなコントロールを要求できる
ように、というものです。そのレジストリは作られませんでした。`value_type` から
導出され*ない* 2 つの描画は、フィールドが登録できるエントリではなく、ページに
組み込まれた構造的な特例です:

- `sensitivity: secret_reference` がシークレットコントロールを選びます。これは
  メタデータをキーにしているので、将来のシークレット葉も自動的に継承します —
  現在レジストリ唯一のシークレット葉は `llm.api_key` です。
- `category` が `Execution mode` のフィールド、またはパスが `rqgm.` で始まる
  フィールドは、汎用テーブルが組み立てられる前に Execution セクション（後述の
  *実行モードは選択可能、ガバナンスのチューニングは今も設定ファイル専用*）へ
  振り分けられます。

コントロールの描画部で特例扱いされるリテラルパスはちょうど 1 つ、モデルカタログの
datalist を受け取る `llm.model` だけです。他のフィールドはすべて上の規則で
描画されます。

設定フィールドを追加する人にとっての帰結: 生成されたコントロールは無料で手に入る
一方、より良いものを登録する場所はありません。スキーマのエントリは per-field の
エディタヒントを運びません — キーは `path`、`value_type`、`default`、`enum`、
`required`、`category`、`level`、`scope`、`sensitivity`、`mutability`、
`applies_when`、`notes`、`source`、`env_override` です — したがって特定のパスに
専用エディタを与えるには、今日では Studio のページコンポーネント自体を編集する
しかありません。だからこそ複合の葉 — `skills`、`resources`、
`evaluator.axis_weights`、`evaluator.custom_axes` — はエディタではなく理由を述べる
無効化された入力として描画されます。値は表示され続け、隠されることはありません;
ただこの画面から編集できないだけです。
...
PATCH のボディの `values` マップはフラットな `{"dotted.path": value}` の部分更新であり、
何かが書かれる前にレジストリに対して検証されます。拒否は `400` として返り、問題のパスが
...
| `mode_interlock_mismatch` | 後述のパス横断ペア検査 — モード意図の片割れだけがあり、対になる相方が一致していない |
...
1. **リポジトリルートの `.env`**（`{ARI}/.env`）。これは*固定の*書き込み先です —
   ライタは一致する `NAME=` 行をその場で置換し、無ければ追記します。
...
**残る 104 のガバナンス葉は読み取り専用のままです。** `Execution mode` カテゴリと
...
**引いたもの** — 現在 104 パス）、自身の `values` にそれらのパスを持つドラフトを
