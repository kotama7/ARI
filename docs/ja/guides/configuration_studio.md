...
**144 葉、メタデータ被覆率 100 %** です — レジストリのビルドは、メタデータの無い
フィールドを出荷するのではなく例外を送出するため、新しい設定フィールドが
category・level・scope・sensitivity・mutability 無しに黙って現れることはできません。

現在のカテゴリ: Governance (96)、Proposal routing (14)、Search (BFTS) (14)、
...
カテゴリで絞り込むので、144 フィールドすべてが発見可能なままです。リゾルバの警告は
...
フィルタボックス横のカウンタは 144 より大きくなることがあります。解決済み
...
**残る 96 のガバナンス葉は読み取り専用のままです。** `Execution mode` カテゴリと
...
**引いたもの** — 現在 96 パス）、自身の `values` にそれらのパスを持つドラフトを
