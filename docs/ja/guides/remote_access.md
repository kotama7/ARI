...
反対側から述べます: ループバック既定も、後述のリモートモードも、**ホスト**の間の
境界です。どちらも権限システムではなく、どこで止まるのかは率直に書いておく価値が
あります。

**どちらのモードにも認可レイヤはありません。** ローカルモードには資格情報が
そもそも存在せず — ループバックバインドでは `resolve_token` が何も返さないため、
リクエストゲートはすべてを素通しします。したがって、そのマシン上でポートへ
ソケットを開けるプロセスはすべて API 全体を握ります: `POST /api/env-keys` による
シークレットの書き込み、チェックポイント内のファイルの上書き、メモリバックエンドの
再起動、そして — 後述の許可を自分自身に発行したうえで — チェックポイントの削除や
全実験プロセスの停止。ループバックバインドはホストを隔てますが、1 つのホストを
共有するユーザーやジョブを隔てはしません。

リモートモードは資格情報を 1 つ足し、そこで止まります。bearer トークンは単一の
全能な値です: 誰の名前でもなく、プロジェクトやランへスコープを絞ることもできず、
自分自身以外の何とも照合されません。権限チェックを取り付けられる境界もまだ
ありません — `/api/v1` の面は仮想の `default` プロジェクトを 1 つだけ公開し、
それ以外のプロジェクト id にはすべて 404 エンベロープを返します。セッション、
ログアウト、セッション単位の失効は存在しません; アクセスの失効とは
`ARI_GUI_TOKEN` を変更してサーバーを再起動することであり、それは全員分を一度に
失効させます。これは見落としではなく決定です:
[GUI-ADR-13](../../adr/gui/GUI-ADR-13-remote-bearer-token.md) はセッション・
ログアウト・失効・マルチユーザーを将来のマルチテナント記録へ恒久的に先送りし、
その consequences で、この単一の全能なトークンをそのまま拡張することはしない、と
述べています。

**確認チャレンジは事故防止の制御であって、アクセス制御ではありません。**
`POST /api/delete-checkpoint` を呼べる者は `POST /api/v1/challenges` も呼べ、
それが要求する許可を自分で発行できます — 発行には、リクエストが既に持っていた
もの以上の資格情報は要りません。2 段階が買うのは、**サーバー**が対象を名指しし、
クライアントがその単回使用の許可をそのまま返さなければならない、という点です。
古くなったタブ、リプレイされたスクリプト、誤ったパスへ向いた一括操作を捕まえるのは
これです。第 2 要素ではありません。語彙も意図的に狭く —
`delete-checkpoint`、`stop-all`、`gpu-monitor-stop` だけです。チェックポイント内の
ファイルの保存・削除・アップロード（`POST /api/checkpoint/file/save`、
`POST /api/checkpoint/file/delete`、`POST /api/checkpoint/{id}/file/upload`）と
メモリバックエンドの再起動（`POST /api/memory/restart`）は、要求されればそのまま
実行されます。

**`viz_access.jsonl` はリクエストログであって、監査証跡ではありません。**
フィールドは後述の*アクセスログ*に挙げた 6 つで、そのうち `client` は対向の
ネットワークアドレスです — つまりどの行も人を名指ししませんし、ループバック既定
では全行が同じループバックアドレスを指します。事後の再構成にとってさらに厄介なのは、
アクティブなチェックポイントが無い間は何も書かれないのに、その状態が、本来記録される
はずの書き込みを止めはしないことです。`POST /api/env-keys` はプロジェクトの `.env`
ファイルを編集し、チェックポイントには一切触れません。よってチェックポイントを
選ばずに書かれたシークレットは、行を 1 つも残しません。行が無いことは、何も
起きなかったことの証拠にはなりません。

以上はダッシュボードを、それが何であるか — 表示しているランの本人が動かす単一
運用者向けの研究ツール — として危険にするものではありません。ただし
「トークンの背後に置いて URL を共有する」がサポートされたマルチユーザー配備では
ないこと、そしてアクセスを持つ全員が全運用者の権限を持つことは意味します。
