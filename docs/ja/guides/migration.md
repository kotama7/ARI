...
1. **Letta サービスを起動する。** `docs/hpc_setup.md#6-letta-memory-backend-deployment`
...
`docs/release_policy.md`) では以下を予定しています:
...
- `docs/refactor_audit.md` — マイグレーション負債の現状。
- `CHANGELOG.md` — リリースごとのノート。
- `ari memory migrate --help` — v0.5 → v0.6 マイグレーターの CLI オプション。
- `docs/howto/troubleshooting.md` — マイグレーション失敗時の対処法。
