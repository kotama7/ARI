...
async def generate_figures_llm(
    ...
    system_prompt = (
        ...
        "  - LAYOUT (a figure with overlapping or truncated text is REJECTED): call "
        "fig.tight_layout() and save with bbox_inches='tight'; put the legend OUTSIDE the axes "
        "(e.g. ax.legend(loc='upper left', bbox_to_anchor=(1.02, 1), borderaxespad=0)) so it "
        "never covers the data; for long x tick labels rotate with rotation=30-45 and ha='right' "
        "and widen figsize so NOTHING truncates; never let tick labels, the legend, data-point "
        "labels, or annotations overlap each other or the plotted data.\n"
        "  - COMPARABILITY: do NOT place values measured on different scales/regimes side by side "
        "without a clear axis or annotation explaining they are not directly comparable.\n"
