...
def test_probe_returns_cached_without_srun(tmp_path):
    cached = {"partition": "partA", "arch": "aarch64", "available": {"perf": False}}
...
def test_cached_probe_cannot_override_status(tmp_path):
    cached = {
        "status": "forged",
        "partition": "partA",
        "arch": "aarch64",
...
def test_probe_skips_gracefully_when_srun_missing(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ARI_SCHEDULER_PATH", str(tmp_path))  # no srun on PATH
    out = asyncio.run(probe_platform_capabilities(str(tmp_path), partition="partA"))
    assert out["status"] == "skipped"
...
def test_probe_rejects_partition_and_tool_injection(tmp_path):
    partition = asyncio.run(
        probe_platform_capabilities(str(tmp_path), partition="cpu;touch-pwn")
    )
    tools = asyncio.run(
        probe_platform_capabilities(str(tmp_path), partition="cpu", tools="perf,$(id)")
    )
    assert partition == {"status": "skipped", "reason": "partition is invalid"}
    assert tools == {"status": "skipped", "reason": "probe tool list is invalid"}


def test_probe_rejects_checkpoint_traversal(tmp_path):
    escaped = str(tmp_path / ".." / tmp_path.name)
    out = asyncio.run(probe_platform_capabilities(escaped, partition="cpu"))
    assert out["status"] == "skipped"
