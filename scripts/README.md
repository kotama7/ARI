...
- `edit_lineage.py` — measures whether each BFTS child REWROTE or REFINED its inherited candidate (the paper's central axis). Runs post-hoc over a checkpoint's persisted node work_dirs — deterministic, no LLM, re-checkable from `workspace + repo`. Pairs each child with its parent (`node_report.json`'s `parent_id`) and compares the two `candidate_*.c` by TEXTUAL token similarity (strip comments → C-tokenize → symmetric difflib ratio): high = edited-in-place (refine), low = regenerated (rewrite). Behavioural semantic-diff is deliberately NOT used — the validity gate forces every valid candidate to compute the same function, so semantic equivalence is constant and cannot separate the two. Optional structural (tree-sitter AST) similarity is reported as an orthogonal robustness check when tree-sitter is installed (versions recorded); it is a weaker discriminator by design. Headline is the threshold-free median; rewrite-rate is reported over a threshold sweep.
