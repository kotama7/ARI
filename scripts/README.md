...
- `rqgm_assurance/` — release utilities for promoting native scientific harnesses and exercising their publication path.
  ...
  - `repin_and_promote_harness.py` — re-pin a registered Harness to the code it is measured by, then register and sign it. A manifest pins three DERIVED values — the driver's content digest, the digest of the result schema ARI ships for the type that driver emits, and its own digest over both — every one computable from the repository, and none computed by anything that wrote a manifest; they were typed in, so an instrument edit silently invalidated the pin and `prepare()` then refused the harness as "driver bytes drifted": registered, signed, unable to run. Measured, that happened three times, the last taking two harnesses at once because `native_perf_common.py` sits in two driver digests. `check` reports stale pins and writes nothing (exit 1 on drift — this is what the pre-commit gate runs); `promote` re-pins, earns the gates and signs. It re-pins DERIVED fields ONLY: the placement, scope, tiers, policies, container, problem and case set are read and never written, because a tool that could move `registered_placement` could relocate a harness's evidence to whatever machine was at hand. It refuses to measure off the pinned placement, refuses a dirty tree, and scrubs host identity out of the environment record at the publication boundary. Distinct from `promote_native_harnesses.py`, which constructs the three native manifests and cannot move an existing pin (`_immutable_outputs` refuses to overwrite differing bytes) — and which raises TypeError at HEAD, because it hands `registration_report` gates with `passed=True` already written into them, which that function stopped accepting at `ccdedc9`.
