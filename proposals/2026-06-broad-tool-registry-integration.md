...
| | |
|---|---|
| **Status** | **Proposed — design converged, NOT build-ready** (see §6) |
| **Date** | 2026-06-08 |
...
> **概要 (Japanese TL;DR)** — ARI を生命科学特化の ToolUniverse 一本より広く、かつ再現性ファーストのまま拡張するための統合設計。適合形は **単一の in-tree stdio ブローカースキル `ari-skill-tool-registry`**（Compact 5 ツール・digest 固定・人手キュレーション catalog・カセット再現層を EAR へ・段階導入 A→B→C・transport は stdio 一本）。ただし**敵対的レビューで現設計は build-ready でない**ことが判明 — 実在しない `ARI_PHASE` への依存、reproduce サンドボックスがスキル/カセットを見られないという 2 つの BLOCKER（いずれも実コードで確認済み）。**実機 spike と 2 点修正が済むまでコードは出さない。** 本 RFC はその設計・根拠・修正項目・spike ゲートを記録するもの。
...
## 6. Build-readiness: **NOT READY** — blockers found by adversarial review (verified against code)

A skeptical review against ARI's actual implementation returned **flawed**. The two blockers below were re-confirmed by directly reading the code and **must be fixed before any implementation PR**:

- **BLOCKER 1 — the design relied on `ARI_PHASE`, which does not exist.** The only `ARI_PHASE*` vars are `ARI_PHASE1_*` (reproduce-sandbox config). Phase is consumed **only** by the driver to filter the tool list (`react_driver.py:252 mcp.list_tools(phase=agent_phase)`); it is **never propagated into a skill subprocess** (the skill env is `{**os.environ, "PYTHONPATH": …}` only). A skill's `server.py` therefore cannot know the current phase, so the "auto-switch to replay on reproduce" mechanism is built on a phantom variable.
  - **Fix**: either (a) pass `mode`/`phase` as an explicit `invoke()` argument the per-phase agent supplies, or (b) add real phase propagation into the skill env in `client.py` — which is a genuine ari-core change and must be reclassified as such (no longer "zero-change").

- **BLOCKER 2 — the reproduce sandbox cannot see the skill or the cassettes.** ARI's reproduce phase runs a self-contained `reproduce.sh` inside an **isolated** sandbox (`run_reproduce`; `build_reproduce_sh` writes into `repro_sandbox/`; `fetch_code_bundle` pre-populates from the curated EAR bundle — `ari-skill-paper-re/src/server.py`). That sandbox has **no ari-core, no MCP skill graph, no broker, no cassettes** unless explicitly vendored in. So "byte-reproducible in the reproduce sandbox via cassette replay through the skill" is hollow as written.
  - **Fix**: vendor frozen cassette payloads into `repro_sandbox/` via `fetch_code_bundle`/`build_reproduce_sh` (so `reproduce.sh` reads them as fixtures **without** the skill), **or** scope the cassette layer to *within-paper-phase* determinism only and drop the sandbox-repro claim.

Additional **must-fix** items (majors/minors):
...
- **EAR bundling**: confirm `ari ear curate` actually bundles `tool_registry_cassettes/` (it is rule-driven via `publish.yaml`); ship an explicit include rule + bundle `catalog.json`/`CATALOG.lock` if not.

> Two of these blockers are "hand-off contract" mismatches (a phase signal that never reaches the skill; artifacts that never reach the sandbox) rather than logic errors — the same class as the failure modes tracked elsewhere in ARI. Catching them in review *before* writing code is the point of this RFC.
...
2. **Resolve the two blockers in the design** (phase signal; reproduce-sandbox vendoring) and update the staging table's "ari-core change" column accordingly.
...
- ARI internals referenced: `ari-core/ari/mcp/client.py` (`_server_params`, `_phase_matches`), `ari-core/ari/agent/react_driver.py:31,252`, `ari-skill-paper-re/src/server.py` (`run_reproduce`/`build_reproduce_sh`/`fetch_code_bundle`), `ari-skill-idea/src/snapshot.py`, `ari-skill-web/src/server.py`
