...
### V2. ミニ run（2-3 node・aarch64 partition・〜40 分）
- 全レイヤー込みで観測:
...
### V3. 本番規模 run（max_nodes=10・x86 か aarch64 partition の空き時・数時間・実費大）
...
- **(c) platform-feasibility（V3 の結果待ち）**: `branch_mpki` 等 PMU 前提の evidence が
  対象 CPU の非特権環境で測定不能と判明した場合、claims 抽出プロンプトに experiment.md の platform 記述を
...
| **P2(c) 実装＋検証** | ✅ de83098 — hpc-skill `probe_platform_capabilities`（compute node 上で command -v 実測→checkpoint へ cache、best-effort）→ evaluator が検証済み能力ノートとして抽出に伝達。**micro-replay: PMU ツール依存 evidence 6→0**（DTLB/LLC は software footprint counts へ、branch は software proxy rate へ再表現、12 claims 全て検証可能なまま） |
...
注: V1 で agent が選んだのは皮肉にも PMU 系（対象 partition で測定不能）——steering は機能するが、platform 不能 claims が誘引になる。P2(c) の優先度を「V3 後」から「次の実装候補」へ引き上げる根拠。
