...
- When all experiments are done, return JSON: {{"status": "success", "metrics": {{...}}, "summary": "...", "next_steps": ["<concrete improvement you would try next>", ...], "concerns": ["<caveat or risk about this result>", ...], "environment": "<the compute environment you actually used>", "file_notes": {{"<filename>": "<one short phrase describing what this file is or what you changed>"}}}} — `next_steps` and `concerns` are your own brief self-review (1-3 items each; [] if genuinely none). `environment` is a short free-text note of the toolchain/hardware your result relied on (e.g. compiler + version, CPU/ISA like AVX-512, GPU, MPI). If `describe_environment` is available, call it first; base `environment` ONLY on real tool output (`describe_environment` / commands you ran) — do NOT fabricate; leave it "" if you did not verify it. `file_notes` maps each file you created or changed to a one-line explanation (so a reader knows what it is without opening it); include only files you actually touched, {{}} if none.
