...
- `assurance/` — scientific harness catalogs, resolution, execution, attestation, registration, and immutable certification locks.
  ...
  - `contract.py` — verification-contract parsing and compatibility checks. `build_verification_contract` takes an optional `artifact_target_kind`: the vocabulary's `target_kinds` is one scalar per property, which was the whole truth while the only correctness harnesses verified a `shared-library`, and is not once the same property is honestly verified on a problem submission too. When a run names a pinned problem it knows which, and that answer overrides the table for the correctness properties only; a run that names none passes nothing and every atom is stamped exactly as before.
  ...
  - `native_perf.py` — public facade for the ARI-native performance harnesses: takes a PINNED PROBLEM REVISION rather than a task name, resolves the frozen reference and its build flags through the problem's registered family, and re-exports `DEFAULT_REGRESSION_THRESHOLD` so the worker and the driver read the ratio from one place instead of defaulting to one each.
  ...
  - `native_perf_measure.py` — the measurement, once, for every problem: anchor/matched denominators, launch ordering, all timed launches before the oracle, per-role output files, and the verdict against the registered regression threshold. Refuses a case set whose family is not the problem's. A shortfall inside the run's own spread is not a regression, and a spread wider than the instrument is READ at resolves neither verdict — that ceiling was missing, so the pass region was `centre >= threshold / (1 + spread)` and grew without limit with the noise, certifying a candidate at 0.5x of the frozen reference. A shortfall past even an unresolvable spread still fails, which is what keeps a slow control refused on a noisy machine. Owns `DEFAULT_REGRESSION_THRESHOLD`, the one ratio the worker, the parity probe and the evaluator all read; it is here rather than beside `MAX_TRUSTED_SPREAD` because `native_perf_common.py` is inside the problem-correctness driver's content address.
  ...
  - `native_problem_correctness.py` — correctness of a candidate against a PINNED PROBLEM's own contract, alongside `native_hpc` rather than instead of it. Both answer "is this kernel right" and both name the interface contract `gemm-c-abi/v1`, and the shared name is the trap: `hpc/gemm-correctness` resolves `ari_gemm_f32`/`ari_gemm_f64` over a 13-argument status-returning ABI in two dtypes across 33 cases, while a candidate submitted to `gemm-dense-fp64` exports `gemm` — six arguments, no status, fp64 only, because that is what `gemm_kernel.h` says. Handing one verifier the other's target is not a near miss: measured, the ARI-native verifier reported 33/33 `missing ari_gemm_f32 symbol` with every error 0.0, so a governed run read a candidate that was never loaded as one that was checked and found wanting. Reuses `native_perf_common`'s build, flag screen, object audit and launch, and the FAMILY's oracle — the same residual bound the performance harness credits time behind — so it is that opinion reported on its own rather than only as a precondition for a stopwatch. Carries no anchor and no denominator: the gemm oracle is a backward-error bound against a NumPy-computed `A @ B`, so correctness needs no competent baseline.
  ...
  - `registration_gates.py` — computes the promotion gates instead of being handed their answers. `registration_report` used to check that the gate ids matched the canonical list and that every `passed` was True, while nothing computed any `passed` — so a registration recorded fifteen satisfied gates, a human signed it, and no gate had been evaluated. One evaluator per gate now, each reading a specific artifact; a gate that cannot be decided from the evidence it was given returns `passed=False` with the reason, never True by default, because answering True without looking is the exact failure being fixed. Three gates are marked `requires_human`: structural checks here, whose real answer is the approval a maintainer signs — marking them is the point, since the old arrangement made them indistinguishable from the twelve a machine can settle. Each gate digests THE ARTIFACT IT READ, so two registrations agree only if they read the same bytes. `result_schema_conformance` also compares the MANIFEST against the driver: a manifest declares `expected_result_schema` and `expected_result_schema_digest` independently, `resolver` copies the digest into the lock, and nothing anywhere read it — so both halves were wrong on the shipped catalog and invisible. One manifest named a report type its driver never emits, and all four pinned `sha256:6ff46e8f…`, which is the schema file's byte digest at `d303a4c`, the commit they were registered from; the file changed afterwards (the `kind` enum was dropped when families became data-driven), so each pins a stricter schema than ARI now ships. A stale pin is refused, not corrected: the pin records what was registered and only a re-registration may move it.
  - `registration_models.py` — promotion approval, evidence, and registration report models.
  - `registration_run.py` — produces the evidence a registration is made of. The gates can be computed only from evidence someone hands them, and nothing in production produced any: `parity_probe`, the one piece of code that establishes an instrument can tell a wrong answer from a slow one, had no caller outside tests, so a gate that CAN be computed and never is sits a short distance from a decorative one. This module is that caller — it runs the probe against the driver being registered, repeats it so stability is measured rather than claimed, reads the commit out of the repository instead of accepting one, and resolves the result schema from the report type the harness actually emits. It refuses a dirty working tree (a pin taken from uncommitted code names a commit that does not describe what ran), a probe run against a different driver, and a stability claim from fewer than two runs. The cost is real: the probe compiles and runs the reference, a slow control and a wrong control, once per repetition. `result_schema_digest` is the ONE place that computes what a manifest's `expected_result_schema_digest` must be — over the schema file's bytes, the same rule `load_tolerance_policy` uses, because a manifest pins the artifact and hashing a reparsed document yields a different digest for the identical file. A hand-copied digest is one only its typist can check.
  - `request.py` — validate and canonicalize screen and certify run requests. `_worker_argv` chooses the worker from the manifest's pinned DRIVER REVISION, which is what the resolver locks and `prepare` re-checks. It built exactly one argv before — the ARI-native worker, with `--kind` derived by string surgery on the harness id (`rsplit('/')[-1].removesuffix('-correctness')`) — which was the whole truth while every artifact verifier was ARI-native. For a problem-pinned correctness harness that produces `--kind gemm-dense-fp64-problem`, naming no registered family, and `--library candidate_gemm.c`, which is C source and not a library: a correctly resolved and correctly locked Harness would have launched the wrong verifier, whose output its own driver cannot parse, and returned `infrastructure_error` on every node. An unknown driver revision now fails closed rather than falling back to the ARI-native worker. The evidence reference's media type likewise comes from the declaration's target kind instead of a hardcoded `application/x-sharedlib`.
  ...
  - `target_abi.py` — load independent, data-driven candidate ABI identities used to author Harness target declarations; adding a family adds a YAML record instead of extending a task-name table in core. Also owns `problem_target_kind(entry_point, family)`, the answer to "what kind of artifact ARE this problem's candidates": a problem whose entry point is one of its family's ABI symbols is verified as that shared library, and anything else keeps its own contract header and is a `benchmark-submission`. Derived from the two pinned facts `declare_target` also reads, so the resolution side and the declaration side cannot come to disagree — before it existed, `property_vocabulary.yaml`'s one-target-kind-per-property table stamped every correctness atom `shared-library`, and a pinned-problem run therefore resolved its correctness obligation to the ARI-native GEMM, SpMM and Stencil verifiers, none of which can load a candidate written against the problem's own header, while the Harness written for that contract was never selected at all.
  - `drivers/` — isolated adapters for native, container, external, shared-library, upstream, and PaperBench harnesses.
    ...
    - `perf_worker.py` — subprocess entry point printing one typed `NativePerfReportV1` on stdout. Mirrors `native_worker` deliberately: a slow or wrong candidate is a SUCCESSFULLY COMPLETED execution whose report says `fail`, and a non-zero process status is reserved for verifier or substrate failure, so infrastructure accounting never conflates the two. Takes the problem and the case set as pinned revisions rather than shapes — registration evidence is established at a size and does not transfer. `--flags` must be passed as `--flags=<value>`: a candidate declaring exactly one flag (`-O3`) is read by argparse as an option and rejected, which would surface as a verifier crash instead of as a candidate result. `--regression-threshold` defaults to `DEFAULT_REGRESSION_THRESHOLD` and is never passed by `_worker_argv`, so this default IS the ratio a registered manifest is judged at — it read 1.0 while the parity probe beside it earned its evidence at 0.95.
    - `problem_correctness.py` — driver for correctness against a PINNED PROBLEM's own contract. Every registered correctness harness verifies the ARI-native ABI out of a shared library; every pinned problem submits C source against its own header. Nothing verified the second, so a governed run over a problem had exactly one correctness harness available and that harness could not load its target — reported not as "no harness fits" but as 33 failing cases. Its digest is its own, and that is why it can exist: putting a problem-shaped verifier inside `native_driver_digest` would have re-registered the three ARI-native harnesses for a file none of them read.
    - `problem_correctness_worker.py` — prints one typed report on stdout, mirroring `perf_worker` deliberately. A candidate that is wrong, or that does not build, is a SUCCESSFULLY COMPLETED verifier execution whose typed report says `fail`; a non-zero process status is reserved for verifier or substrate failure, so infrastructure accounting never conflates the two.
...
- `rqgm/` — Constitutional ARI-RQGM runtime: the opt-in `ari_rqgm` execution mode (epoch governance and co-evolution). Internal package — never exported via `ari.public.*`, never imported on the default `simple_bfts` path (`ari.core.build_runtime` imports it lazily only when `ari.mode: ari_rqgm` and `rqgm.enabled: true` agree).
  ...
  - `paper_ablation.py` — RQGM-paper P0-P4 mechanism postures and ordinary-switch consistency checks, read by the paper runtime DURING a run (`posture_from_config`, `PaperAblationPosture.role_evolution_enabled`, `config_violations`). P0 gates successor generation to `paper_writer` (fixed critic); P0-P3 retain the kernel in nonblocking `audit_only`, while P4 uses standard enforcement. Inert unless `rqgm.eval.enabled` is true. Production, NOT `evaluation/`: it lived there and made the ordinary `ari paper` path import the Task-13/20 harness at five sites; nothing inside that package ever imported it, and its config field is declared in `ari.config`.
...
- `schemas/` — JSON Schemas shipped with ari-core.
  ...
  - `native_problem_correctness_report_v1.schema.json` — the typed report the problem-correctness worker prints and its driver normalizes: per-case verdicts carrying the worst ratio to the family's residual bound (1.0 is the bound), the expected and written output element counts, and per-property verdicts for `numerical-equivalence` and `interface-conformance` so a run learns which of the two claims a candidate broke. Generated from `NativeProblemCorrectnessReportV1`; registration's result-schema-conformance gate resolves this file from the driver's declared report type and fails outright when it is absent. Carries no seconds and no speedup — a correctness verdict quoting a time would be read as a performance result taken without a denominator.
