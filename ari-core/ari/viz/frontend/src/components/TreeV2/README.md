...
- shareable URL selection: `?node=` is read from and written back to the
  hash query, so "look at node N7 of run R1" is a link rather than a sequence
  of clicks;
...
- large-tree level-of-detail (task 07 tail) — a reduction of what is
  RENDERED, never of what was fetched: above
  ...
  "showing N of M nodes (depth-limited)" banner offers [expand all], because
  a view showing less than it has must say so;
- a virtualized side table (`TreeTablePanel.tsx`): the same visible rows as
  ...
  `?node=` — an equivalent tabular hierarchy, so nothing in the workspace is
  reachable only by pointing at the canvas. Canvas clicks and table selection
  share the `?node=` URL state.
