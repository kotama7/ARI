...
- ``conditions`` — B0-B8 exploration, B paper-archive, and RQGM-paper-aligned
  ...
  workflow-overlay conversion, feature-flag flattening) plus the VirSci-off
...
- ``injection`` — the ten failure injections of
  docs/guides/rqgm_evaluation.md, "Failure injections": spec
  loading/validation (``eval_*`` namespace), deterministic idempotent
  fixture application, the ``rqgm_injection_provenance.json`` marker, and the
  claim-gate detection helper for fixture injections 1/2.
