...
without a profile):
  ...
  Field semantics (mirror ``replication_rubric.schema.json``):
    ...
    - accepts_reduced_scale (default true): allow smaller-scale runs for
      partial credit.
    - SLURM hints (compiled by paper-re into the typed HPC job contract when present;
      ...
      module_loads (e.g. ["cuda/12.4","openmpi/4.1"]), account, qos,
      reservation. Reject every arbitrary scheduler flag or
      scheduler escape hatch.
